...
pushOperation :: Operation -> Emit String
pushOperation = \case
...
pushChain :: Chain -> Emit ()
pushChain Chain { operation, continuation } = do
  environment <- use (the @"environment")
  
  forM_ (decompose $ consumes operation) $ \(variable, variables) -> do
    let 
  ...
  name <- pushOperation operation

  case instantiate [wrap name] <$> continuation of
    Just chain -> do
      when (name `notElem` consumes chain) $ do
        pushCleanup name
...
commitChain :: [String] -> Chain -> Compile [String]
commitChain environment chain = runEmit (pushChain chain) environment

prepareBlock :: (String, Scope Chain) -> Compile ([String], Chain)
...
commitBlock :: ([String], Chain) -> Compile ()
commitBlock (args, body) = do
  startCode

  cleanups <- commitChain [] body
...
prepareClosure :: (String, Scope (Scope Chain)) -> Compile ([String], [String], Chain)
prepareClosure (name, envScope) = do
  let
    ...
    chain = instantiate (map wrap args) argScope
  
  declareFunc ("_closure_" ++ name) (("envs", i32) : [(arg, i32) | arg <- args]) [i32]
  
  return (envs, args, chain)

commitClosure :: ([String], [String], Chain) -> Compile ()
commitClosure (envs, args, chain) = do
  startCode

  cleanups <- commitChain envs chain

  forM_ (filter (`notElem` consumes chain) args ++ cleanups) $ \cleanup -> do
    pushLocalGet cleanup
    pushCall "leave"
